...
bignum operator/(const bignum& div, const bignum& dsor)
{
	...
	for(int i = div.dim - 1; i >= 0; i--)
	{
		if(dividend >= divisor)
		{
			unsigned short quotient = dividend.calc_coc(divisor);
			total_quotient.digits[0] = quotient;
			bignum substraction(dividend.dim);
			substraction = divisor * quotient;
			substraction.dim = resize(substraction.digits, substraction.dim);
			dividend = dividend - substraction;
		}
		else
